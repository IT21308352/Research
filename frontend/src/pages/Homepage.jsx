  ...
  },
 {
  ...
  return (
    <div className="relative min-h-screen font-sans text-gray-100">
      ...
      {/* Navbar */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-black/80 shadow-lg backdrop-blur-sm' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              ...
              {/* App Name */}
              <div className="ml-4 text-xl font-bold bg-gradient-to-r from-green-400 to-emerald-500 bg-clip-text text-transparent">
                {appName}
              </div>
            </div>
            
            {/* Navigation Links - Empty for simplicity as per requirements */}
          </div>
        </div>
      </nav>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="py-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            {features.map((feature, index) => (
              <div 
                key={index}
                className={`bg-gray-900/60 backdrop-blur-sm p-6 rounded-xl border border-gray-800 hover:border-green-700 transform hover:-translate-y-2 transition-all duration-300 shadow-lg hover:shadow-xl cursor-pointer overflow-hidden group ${showCards ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
      ...
      {/* Footer */}
      {/* <footer className="relative z-10 bg-black/40 backdrop-blur-sm py-8 border-t border-gray-800">
